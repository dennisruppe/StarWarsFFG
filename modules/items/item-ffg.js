  ...
  getItemDetails() {
    ...
    // General equipment properties
      ...
      if (data.hasOwnProperty("adjusteditemmodifier")) {
        const qualities = data.adjusteditemmodifier?.map((m) => `<li class='item-pill ${m.adjusted ? "adjusted hover" : ""}' data-item-id='${this.id}' data-uuid='${this.uuid}' data-modifier-id='${m.id ?? m._id}' data-modifier-type='${m.type}'>${m.name} ${m.data?.rank_current > 0 ? m.data.rank_current : ""} ${m.adjusted ? "<div class='tooltip2'>" + game.i18n.localize("SWFFG.FromAttachment") + "</div>" : ""}</li>`);
